        ...
        databaseHooks: (() => {
          ...
          for (const adapter of adapters) {
            if (adapter.relatedModels) {
              for (const [model, getRelationId] of Object.entries(adapter.relatedModels)) {
                if (!hooks[model]) {
                  hooks[model] = {}
                }

                const triggerParentSync = async (data: Record<string, unknown>) => {
                  ...
                  if (relationId) {
                    ...
                    if (parent) {
                      await sendWebhookEvent("update", parent, adapter, ctx, opts)
                    }
                  }
                }
