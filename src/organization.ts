  ...
  listOrganizationInvitations: createAuthEndpoint(
    ...
    async (ctx) => {
      ...
      try {
        ...
        const inviterIds = [
          ...new Set(
            activeInvitations.map((inv) => inv.inviterId).filter((id): id is string => !!id)
          ),
        ]
        ...
        if (inviterIds.length) {
          const inviters = await Promise.all(
            inviterIds.map((id) => ctx.context.internalAdapter.findUserById(id))
          )
  ...
  createOrganizationInvitation: createAuthEndpoint(
    ...
    async (ctx) => {
      ...
      try {
        ...
        const now = new Date()
        const validInvitations = existingInvitations.filter((inv) => new Date(inv.expiresAt) > now)
